...
subString start end = take (end + 1 - max 0 start) . drop start
...
lowerAndGreater n = go 0 0
  where go :: Int -> Int -> [Int] -> String
        go lowest greatest [] = show n ++ " is greater than " ++ show lowest ++ " elements and lower than " ++ show greatest ++ " elements"
        go lowest greatest (x:xs)
          | x > n = go lowest (greatest + 1) xs
          | x < n = go (lowest + 1) greatest xs
          | otherwise = go lowest greatest xs
